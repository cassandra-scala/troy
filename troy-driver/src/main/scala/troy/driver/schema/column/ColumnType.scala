 ...
 */
trait ColumnType[Version, Table <: TableName[_, _], Column <: Identifier] {
  type Out <: CassandraDataType
}

object ColumnType {
  type Aux[V, T <: TableName[_, _], C <: Identifier, CT] = ColumnType[V, T, C] { type Out = CT }

  def instance[V, T <: TableName[_, _], C <: Identifier, CT <: CassandraDataType](implicit tableExists: TableExists[V, T]): Aux[V, T, C, CT] =
    new ColumnType[V, T, C] { type Out = CT }
}
